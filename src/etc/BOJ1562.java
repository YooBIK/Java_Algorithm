...
public class BOJ1562 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= 9; i++) {
            dp[1][i][1 << i] = 1;    // [자리수][끝나는수][사용한수들 bitmask] [1][2][011(2진수)] 1자리수 중 2로끝나고, 0과 1을 사용한 경우
        }

        for (int i = 2; i <= N; i++) {
            for (int k = 0; k < 1024; k++) {
                dp[i][0][k | 1] += dp[i - 1][1][k] % 1000000000; // 길이 i-1인 1로 끝나는 수의 개수 중 [k] 는 사용한 숫자들의 경우에 해당하는
